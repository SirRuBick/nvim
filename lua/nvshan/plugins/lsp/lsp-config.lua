...
Config.on_attach = function(client, bufnr)
  ...
  if is_available "nvim-lint" then
    ...
    ):buffer(bufnr):desc("Lint with options")
  end
  ...
  if is_available "lspsaga.nvim" then
    ...
    -- other keymaps
    if lsp_mappings.n["[d"] then lsp_mappings.n["[d"].rhs = "<CMD>Lspsaga diagnostic_jump_prev<CR>" end
    if lsp_mappings.n["]d"] then lsp_mappings.n["]d"].rhs = "<CMD>Lspsaga diagnostic_jump_next<CR>" end
    if lsp_mappings.n["<leader>ld"] then lsp_mappings.n["<leader>ld"].rhs = "<CMD>Lspsaga show_line_diagnostics<CR>" end
    if lsp_mappings.n["<leader>lr"] then lsp_mappings.n["<leader>lr"].rhs = "<CMD>Lspsaga rename<CR>" end
    if lsp_mappings.n["K"] then lsp_mappings.n["K"].rhs = "<CMD>Lspsaga hover_doc<CR>" end
    if lsp_mappings.n["<leader>la"] then lsp_mappings.n["<leader>la"].rhs = "<CMD>Lspsaga code_action<CR>" end
    if lsp_mappings.v["<leader>la"] then lsp_mappings.v["<leader>la"].rhs = "<CMD>Lspsaga code_action<CR>" end
    if lsp_mappings.n["gd"] then lsp_mappings.n["gd"].rhs = "<CMD>Lspsaga go_to_definition<CR>" end
    if lsp_mappings.n["gT"] then lsp_mappings.n["gT"].rhs = "<CMD>Lspsaga goto_type_definition<CR>" end
  end

  if is_available "telescope.nvim" then -- setup telescope mappings if available
    if lsp_mappings.n["<leader>ls"] then lsp_mappings.n["<leader>ls"].rhs = "<CMD>Telescope document_symbol<CR>" end
    if lsp_mappings.n["<leader>lc"] then lsp_mappings.n["<leader>lc"].rhs = "<CMD>Telescope lsp_incoming_calls<CR>" end
    if lsp_mappings.n["<leader>lC"] then lsp_mappings.n["<leader>lC"].rhs = "<CMD>Telescope lsp_outgoing_calls<CR>" end
    if lsp_mappings.n["gd"] then lsp_mappings.n["gd"].rhs = "<CMD>Telescope lsp_definitions<CR>" end
    if lsp_mappings.n["gI"] then lsp_mappings.n["gI"].rhs = "<CMD>Telescope lsp_implementations<CR>" end
    if lsp_mappings.n["gr"] then lsp_mappings.n["gr"].rhs = "<CMD>Telescope lsp_references<CR>" else vim.notify("no gr map") end
    if lsp_mappings.n["gT"] then lsp_mappings.n["gT"].rhs = "<CMD>Telescope lsp_type_definitions<CR>" end
